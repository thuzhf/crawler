...
class Worker(mp.Process):
    ...
    def __init__(self, job_params_queue, results_queue, necessary_resources, handler_func):
        ...
        # if callable(necessary_resources):
    ...
    def run(self):
        ...
        if self.necessary_resources:
            if callable(self.necessary_resources):
                self.necessary_resources = self.necessary_resources()
            else:
                self.necessary_resources = necessary_resources
            ...
            try:
                self.necessary_resources.close()
            except:
                pass
            try:
                self.necessary_resources.quit()
            except:
                pass
        else:
            for job_params in iter(self.job_params_queue.get, None):
...
class MPRoutine(object):
    ...
    """
    def __init__(self, handler_func, necessary_resources=None, num_workers=0):
